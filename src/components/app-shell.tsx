...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const timeout = setTimeout(async () => {
      ...
      const { data: students } = await supabase.from("students").select("id, name, reg_no, course, level").or(`name.ilike.%${q}%,reg_no.ilike.%${q}%`).limit(5);
    ...
    <div className="min-h-screen flex w-full bg-background">
      <aside className={cn("fixed inset-y-0 left-0 z-30 flex flex-col bg-sidebar text-sidebar-foreground transition-all duration-300 ease-in-out", collapsed ? "w-20" : "w-64")}>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {visibleMain.map((item) => <NavItem key={item.to} {...item} active={pathname === item.to} collapsed={collapsed} />)}
          {visibleAdmin.length > 0 && (
            <div className="pt-6 pb-2">{!collapsed && <p className="px-3 text-[10px] tracking-[0.2em] text-sidebar-muted font-medium">ADMIN</p>}</div>
          )}
          {visibleAdmin.map((item) => <NavItem key={item.to} {...item} active={pathname === item.to} collapsed={collapsed} />)}
        </nav>
        ...
        <header className="sticky top-0 z-20 h-16 bg-card/80 backdrop-blur border-b flex items-center gap-4 px-6">
          <div className="relative flex-1 max-w-xl" ref={searchRef}>
            <form onSubmit={(e) => {
              ...
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onFocus={() => searchResults.length > 0 && setShowDropdown(true)} placeholder="Search students by name or reg no..." className="w-full h-10 pl-10 pr-10 rounded-lg bg-muted border border-transparent text-sm placeholder:text-muted-foreground focus:outline-none focus:bg-card focus:border-input transition-all" />
            ...
            {showDropdown && searchResults.length > 0 && (
              <div className="absolute top-12 left-0 right-0 bg-card border rounded-lg shadow-lg z-50 overflow-hidden">
                <div className="p-2 border-b bg-muted/50"><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide px-1">Students</p></div>
                <div className="max-h-80 overflow-y-auto">
                  {searchResults.map((s) => (
                    <button key={s.id} onClick={() => handleSearchSelect(s)} className="w-full flex items-center gap-3 px-3 py-2.5 text-left hover:bg-accent transition-colors">
                      ...
                      <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{s.name}</p><p className="text-xs text-muted-foreground truncate">{s.reg_no} • {s.course} • {s.level}</p></div>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {showDropdown && !isSearching && searchQuery.trim().length >= 2 && searchResults.length === 0 && (
              <div className="absolute top-12 left-0 right-0 bg-card border rounded-lg shadow-lg z-50 p-4 text-center"><p className="text-sm text-muted-foreground">No students found for "{searchQuery}"</p></div>
            )}
          </div>
          <button onClick={toggleDark} className="h-10 w-10 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground transition-all" aria-label="Theme">{dark ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}</button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild><button className="relative h-10 w-10 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground transition-all" aria-label="Notifications"><Bell className="h-5 w-5" /><span className="absolute top-1.5 right-1.5 h-4 w-4 rounded-full bg-destructive text-destructive-foreground text-[10px] font-semibold flex items-center justify-center">1</span></button></DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-80"><DropdownMenuLabel>Notifications</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem className="flex flex-col items-start gap-0.5"><span className="text-sm font-medium">Welcome to Sandstone</span><span className="text-xs text-muted-foreground">Your dashboard is ready.</span></DropdownMenuItem></DropdownMenuContent>
          </DropdownMenu>
          <DropdownMenu>
            <DropdownMenuTrigger asChild><button className="flex items-center gap-2 pl-2 pr-3 h-10 rounded-lg hover:bg-muted transition-all"><div className="h-8 w-8 rounded-full bg-sidebar text-sidebar-foreground flex items-center justify-center"><span className="text-sm font-semibold">{role ? role[0].toUpperCase() : "?"}</span></div><span className="text-sm font-medium hidden sm:block">{role ? ROLE_LABELS[role] : "—"}</span></button></DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56"><DropdownMenuLabel>{role ? ROLE_LABELS[role] : "Account"}</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onClick={() => navigate({ to: "/settings" })}><Settings className="h-4 w-4 mr-2" /> Settings</DropdownMenuItem><DropdownMenuItem onClick={() => navigate({ to: "/change-password" })}><UserCog className="h-4 w-4 mr-2" /> Change Password</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={handleLogout} className="text-destructive focus:text-destructive"><LogOut className="h-4 w-4 mr-2" /> Log out</DropdownMenuItem></DropdownMenuContent>
          </DropdownMenu>
        </header>
...
function NavItem({ to, label, icon: Icon, active, collapsed }: { to: string; label: string; icon: any; active: boolean; collapsed: boolean }) {
  return (
    <Link to={to} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-300", active ? "bg-sidebar-active text-sidebar-foreground shadow-lg" : "text-sidebar-foreground/80 hover:bg-white/10 hover:text-sidebar-foreground hover:translate-x-1")}>
      <Icon className="h-5 w-5 shrink-0" />{!collapsed && <span className="truncate">{label}</span>}
    </Link>
  );
}